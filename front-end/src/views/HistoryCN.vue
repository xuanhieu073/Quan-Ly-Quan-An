      ...
      <b-row >
        <b-col cols="4">
          <b-card no-body header-tag="header" footer-tag="footer" class="shadow">
            ...
            <b-list-group>
              <b-list-group-item v-for="item in list" :key="item.ProID"  :to="`/historycn/1/${item.Id}`">
                <b-row>
...
export default {
    ...
    methods: {
        ...
        async donhang(){
          const res = await axios.get(`http://localhost:3000/donhang/chinhanh/${this.user.payload.f_ChiNhanhId}/ban/${this.banid}`,{headers : {'x-access-token':this.user.token}})
          console.log(res)
          this.list = res.data
        },
