...
export default {
  ...
  methods: {
    ...
    fetchProducts(catId) {
      axios
        .get(`http://localhost:3000/categories/${catId}/chinhanh/${this.user.payload.f_ChiNhanhId}/products`,{headers : {'x-access-token':this.user.token}})
        .then(res => {
          this.list = res.data;
    ...
    addcart(food){
      const found = this.tempcart.find(e => e.ProID == food.ProID && e.banid == this.$route.params.chinhanhId)
      if(found){
        ...
        food.chinhanhId = this.user.payload.f_ChiNhanhId;
        food.banid = parseInt(this.$route.params.chinhanhId);
